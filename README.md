...
│   ├── mcp/             # MCP サーバー（AI からの .slide.json 操作）
...
projects/
  my-talk/
    my-talk.slide.json       # スライドデータ（playback 設定含む）
    audio/
      slide-00.wav           # VOICEVOX 生成ナレーション
      slide-01.wav
    bgm.mp3                  # BGM ファイル
    my-talk.mp4              # 出力動画
```

動画エクスポート時にナレーションを VOICEVOX で合成すると、WAV ファイルがプロジェクトディレクトリに保存され、`.slide.json` の `playback` 設定に `audioFile` パスが自動追記される。2 回目以降のレンダリングでは既存の音声ファイルが再利用される。
