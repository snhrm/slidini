...
export async function prepareAudio(
	...
	for (let i = 0; i < presentation.slides.length; i++) {
		...
		if (slideConfig?.audioFile) {
		...
		} else if (slideConfig?.narration && voicevoxClient && config.voicevox) {
			// Synthesize narration via VOICEVOX and save to configDir
			...
			const wavFilename = `narration-${String(i).padStart(3, "0")}.wav`
			const wavPath = path.join(configDir, wavFilename)
			fs.writeFileSync(wavPath, wavBuffer)

			// Update config with audioFile path
			slideConfig.audioFile = wavFilename
			configUpdated = true
