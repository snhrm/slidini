...
function readPresentation(filePath: string): Presentation {
...
function writePresentation(filePath: string, presentation: Presentation): void {
	const absPath = resolve(filePath)
...
function findSlideByIndex(presentation: Presentation, index: number) {
...
Returns: Confirmation with remaining slide count.`,
	...
	async ({ file_path, slide_id }) => {
		try {
			...
			writePresentation(file_path, presentation)
			return ok(
				JSON.stringify({ removed: slide_id, remainingSlides: presentation.slides.length }, null, 2),
			)
		} catch (e) {
...
Returns: Confirmation of removal.`,
	...
	async ({ file_path, slide_id, element_id }) => {
		try {
			...
			return ok(
				JSON.stringify(
					{ removed: element_id, slideId: slide_id, remainingElements: slide.elements.length },
					null,
					2,
				),
			)
...
Returns: The new slide's id and its index.`,
	...
	async ({ file_path, template_id, insert_at }) => {
		try {
			...
			return err(`Error adding slide from template: ${e instanceof Error ? e.message : String(e)}`)
		}
	},
)
...
Returns: Confirmation with the applied color set details.`,
	...
	async ({ file_path, color_set_id }) => {
		try {
			...
			writePresentation(file_path, presentation)
			const result = { colorSetId: color_set_id, slidesUpdated: presentation.slides.length }
			...
			if (element_type === "text") {
				...
				element = createDefaultImageElement({
					...
					...(opts.x != null || opts.y != null
						...
						: {}),
					...(opts.width != null || opts.height != null
						? { size: { width: opts.width ?? 1000, height: opts.height ?? 700 } }
						: {}),
					...(opts.opacity != null ? { opacity: opts.opacity } : {}),
				})
			}
